...
export const createResignation = async (req, res) => {
  ...
  const closeSession = async (abort = false) => {
    if (!session || transactionFinished) {
      return;
    }
    if (abort) {
  ...
  try {
    ...
    // Start transaction only when we are about to write
    ...
    await closeSession(!transactionCommitted);
